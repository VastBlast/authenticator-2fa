...
import { DEFAULT_SETTINGS, type AppSettings, type AuthenticatorAccount } from '../../src/lib/auth/types';
import {
...
describe('AuthenticatorVault persistence and locking', () => {
  ...
  test('uses the browser UI language for a new vault and persists it with the first change', async () => {
    installMemoryStorage({ uiLanguage: 'fr-CA' });
    const vault = new AuthenticatorVault();
  ...
  test('preserves a saved language when the browser UI language differs', async () => {
    installMemoryStorage({ uiLanguage: 'de-DE' });
